...
public class Walker : Behaviour {
	...
	public override void Init(){
		...
		target = GameObject.Find ("rainbowMan_v6");
...
#region Idle
	...
	public override bool Attacking(){
		...
		if(target != null){
			...
			if(Mathf.Abs(dx) > 2f && Mathf.Abs(dx) < 4f) Jump();
			f *= Time.deltaTime * 100;
	...
	public bool IsGrounded(){
		return Physics2D.Raycast (rigidbody2D.position, -Vector2.up, renderer.bounds.size.y/1.5f);
	}
